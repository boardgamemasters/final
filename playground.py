...
if st.session_state['user_login']==True:
    ...
    with st.container():
        ...
        for i in range(0, ncol, 5):
            ...
            with col1:
                # st.image(user_games.iloc[i]['image'])
                st.image(pred.make_square(st.image(user_games.iloc[i]['image'])))#, w=400, h=400))
                st.text(user_games.iloc[i]['name'])
            with col2:
                if i + 1 < ncol:
                    # st.image(user_games.iloc[i+1]['image'])
                    st.image(pred.make_square(st.image(user_games.iloc[i+1]['image'])))#, w=400, h=400))
                    st.text(user_games.iloc[i+1]['name'])                    
            with col3:                 
                if i + 2 < ncol:
                    # st.image(user_games.iloc[i+2]['image'])
                    st.image(pred.make_square(st.image(user_games.iloc[i+2]['image'])))#, w=400, h=400))
                    st.text(user_games.iloc[i+2]['name'])
            with col4:                 
                if i + 3 < ncol:
                    # st.image(user_games.iloc[i+3]['image'])
                    st.image(pred.make_square(st.image(user_games.iloc[i+3]['image'])))#, w=400, h=400))
                    st.text(user_games.iloc[i+3]['name'])
            with col5:                 
                if i + 4 < ncol:
                    # st.image(user_games.iloc[i+4]['image'])
                    st.image(pred.make_square(st.image(user_games.iloc[i+4]['image'])))#, w=400, h=400))
                    st.text(user_games.iloc[i+4]['name'])
else:
    st.write('') 